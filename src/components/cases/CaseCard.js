...
function CaseCard({data, index}){
    return(
        ...
        key={data.title} className="hidden md:flex flex-col overflow-hidden rounded-lg shadow-lg">
        ...
        <div className="flex flex-1 flex-col justify-between bg-white p-6">
          <div className="flex-1">
            ...
            <a href={data.href} className="mt-2 block">
              <p id={data.title} className="lg:text-4xl pt-4 pb-6 text-2xl font-semibold transition duration-400 ease-in-out text-gray-900">{data.title}</p>
              <p className="mt-3 space-y-2 leading-7 text-gray-500">{data.description}</p>
            </a>
          </div>
