...
function CategoriesHeader({ categories }) {
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        setTimeout(() => navigate("/blog/search/" + term), 0.2);
        setTerm("");
    };
    ...
    return (
        <div className="w-full bg-gray-100 py-8">
            <div className="grid grid-cols-12">
                                    ...
                                    </Link>
                                    {
                                        categories && categories.map((category, index) => (
                                            <Link key={index} to={`/category/${category.slug}`}
                                                className={`${location.pathname === `/category/${category.slug}` ? "text-orange-500 bg-white" : "text-gray-900  hover:text-orange-500 border border-gray-100 hover:border-gray-200"} py-2 px-6 rounded-md text-lg font-regular`}>
                                                {category.name}
                                            </Link>
                                        ))
                                    }
                ...
                <form onSubmit={(e) => onSubmit(e)} className="relative col-span-4 md:col-span-2 mr-1 md:mr-8">
