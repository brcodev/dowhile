...
function BlogList({posts,get_blog_list_page,count, term, category}){

    return(
    <div className="overflow-hidden px-8 bg-white">
      <ul role="list" className="divide-y space-y-8 gap-8  divide-gray-200">
        {posts&&posts.map((post,index) => (
          category ? <BlogCardHorizontal data={post} key={index} index={index}/> : <BlogCardSearch data={post} key={index} index={index}/> 
        ))}
      </ul>
